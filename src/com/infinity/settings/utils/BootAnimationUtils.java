 ...
 */
package com.infinity.settings.utils;
...
public class BootAnimationUtils {
    ...
    public static String getSelectedBootAnimation() {
        File customBootAnim = new File(BOOT_ANIMATION_FILES[0]);
        if (customBootAnim.exists() && customBootAnim.canRead()) {
            Log.d(TAG, "Using custom boot animation: " + BOOT_ANIMATION_FILES[0]);
            return BOOT_ANIMATION_FILES[0];
        }

        final int style = getBootAnimStyle();
        if (style >= 0 && style < BOOT_ANIMATION_FILES.length) {
            File selectedFile = new File(BOOT_ANIMATION_FILES[style]);
            if (selectedFile.exists() && selectedFile.canRead()) {
                Log.d(TAG, "Falling back to style-selected boot animation: " + BOOT_ANIMATION_FILES[style]);
                return BOOT_ANIMATION_FILES[style];
            } else {
                Log.w(TAG, "Selected boot animation file " + BOOT_ANIMATION_FILES[style] + " not found or unreadable");
            }
        } else {
            Log.w(TAG, "Invalid boot animation style: " + style);
        }
        return null;
    }

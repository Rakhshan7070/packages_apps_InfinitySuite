...
public class AnimateWallpaperView extends ImageView {
    ...
    private long animationDuration = 130000L;
    ...
    private void startSmoothAnimation() {
        ...
        animator.addUpdateListener(animation -> {
            ...
            float scale = 1 + 0.1f * (float) Math.sin(fraction * Math.PI * 2);

            viewportRect.set(centerX - dx - (getWidth() / (2 * zoomFactor * scale)),
                             centerY - dy - (getHeight() / (2 * zoomFactor * scale)),
                             centerX - dx + (getWidth() / (2 * zoomFactor * scale)),
                             centerY - dy + (getHeight() / (2 * zoomFactor * scale)));
            invalidate();
        });
